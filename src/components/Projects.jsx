    ...
    >
      <Reveal>
        <div className="mb-8">
          ...
          <h2 className="mt-2 text-4xl md:text-5xl font-bold text-[#10304A] dark:text-white">Selected Work & Case Studies</h2>
        </div>
          ...
          {projects.map((project, index) => (
            <div key={index} className="group bg-[#E2D0B6]/80 dark:bg-slate-900/50 backdrop-blur-xl border border-white/40 dark:border-white/10 rounded-[2rem] overflow-hidden shadow-xl shadow-[#10304A]/10 dark:shadow-black/40 hover:shadow-2xl hover:shadow-[#993131]/20 dark:hover:shadow-cyan-500/20 hover:-translate-y-2 transition-all duration-500 flex flex-col h-full">
              ...
              <div className="p-6 flex flex-col flex-grow">
                <h3 className="text-xl font-bold text-slate-900 dark:text-white mb-2 group-hover:text-[#0d96ab] transition-colors">{project.title}</h3>
                ...
                <div className="flex flex-wrap gap-2 mb-6">
                  {project.tech.map((tech, i) => (
                    <span key={i} className="text-xs font-semibold tracking-wide bg-[#0d96ab]/10 text-[#0d96ab] px-3 py-1 rounded-full border border-[#0d96ab]/20 whitespace-nowrap">{tech}</span>
                  ))}
                </div>
